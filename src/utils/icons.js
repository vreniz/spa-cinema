// src/utils/icons.js
// A tiny inline-SVG icon set (outline style). Using inline SVG avoids any icon
// font dependency, scales crisply, and inherits the current text color via
// `stroke="currentColor"`, so icons automatically follow the theme.
...
// `classes` lets callers size/color it with Tailwind utilities.
export function icon(name, classes = "w-5 h-5") {
  const body = PATHS[name] || "";
  return `<svg class="${classes}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${body}</svg>`;
}
